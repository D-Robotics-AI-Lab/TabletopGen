...
FIT_MARGIN_CM = 2.0 # Safety margin when enlarging the table (cm)
ADAPT_POSES_WITH_TABLE_RESIZE = True # Whether to proportionally adjust object positions after table enlargement
...
def load_fused_mesh(glb_path: str) -> "trimesh.Trimesh":
    ...
    for node_name, geom_name in pairs:
        ...
        try:
            T = scene.graph.get(node_name)[0]  # Common usage: returns (matrix, name)
        except Exception:
...
def _rotate_points(points_xy: np.ndarray, angle_degrees: float) -> np.ndarray:
    """
    Rotate a set of 2D points around the origin by angle_degrees (angle inverted to match create_rotated_rectangle).
    """
    angle_rad = np.radians(-angle_degrees)
...
def rescale_poses_for_table_resize(old_main_size, new_main_size, main_object_name, rotation_data, poses):
    """
    Anisotropically scale relative positions of all objects according to table size change ratio in table's local coordinate system, then rotate back to world system.
    """
    old_L, old_W = float(old_main_size[0]), float(old_main_size[1])
    new_L, new_W = float(new_main_size[0]), float(new_main_size[1])
    if old_L <= 0 or old_W <= 0:
        return poses

    sx, sy = new_L / old_L, new_W / old_W
    table_rot = float(rotation_data.get(main_object_name, 0.0))

    new_poses = {}
    for obj_name, (x, y) in poses.items():
        if obj_name == main_object_name:
            new_poses[obj_name] = [0.0, 0.0]
            continue
        # World -> Table local
        p_local = _rotate_points(np.array([[x, y]], dtype=float), table_rot)[0]
        # Local proportional scaling
        p_local_scaled = np.array([p_local[0] * sx, p_local[1] * sy], dtype=float)
        # Local -> World (inverse rotation)
        p_world = _rotate_points(p_local_scaled.reshape(1, 2), -table_rot)[0]
        new_poses[obj_name] = [round(float(p_world[0]), 2), round(float(p_world[1]), 2)]
    return new_poses

def fit_table_to_objects(adjusted_main_size, main_object_name, size_data, rotation_data, poses, margin_cm=FIT_MARGIN_CM):
    """
    If any object's rotated rectangle exceeds the table, enlarge table length/width to just fit (plus margin).
    """
    table_L, table_W, table_H = adjusted_main_size
    table_rot = float(rotation_data.get(main_object_name, 0.0))

    # Collect corners of all objects (transform to table local coordinate system)
    xs, ys = [], []
    for obj_name, (x, y) in poses.items():
        if obj_name == main_object_name:
            continue
        if obj_name not in size_data:
            continue  # Skip if no size to generate rotation box
        obj_L, obj_W = float(size_data[obj_name][0]), float(size_data[obj_name][1])
        obj_rot = float(rotation_data.get(obj_name, 0.0))

        # First generate rotated rectangle corners of the object in world coordinate system
        corners_world = create_rotated_rectangle(x, y, obj_L, obj_W, obj_rot)
        # Transform to table local coordinate system (counteract table rotation)
        corners_table_local = _rotate_points(corners_world, table_rot)

        xs.extend(corners_table_local[:, 0])
        ys.extend(corners_table_local[:, 1])

    if not xs or not ys:
        # No objects available for judgment, return directly
        return adjusted_main_size, False

    max_abs_x = max(abs(v) for v in xs)
    max_abs_y = max(abs(v) for v in ys)

    need_L = 2.0 * max_abs_x + margin_cm
    need_W = 2.0 * max_abs_y + margin_cm

    new_L = max(float(table_L), float(need_L))
    new_W = max(float(table_W), float(need_W))

    changed = (new_L > table_L + 1e-6) or (new_W > table_W + 1e-6)
    if changed:
        print(f"[Auto-Fit] Table extension: L {table_L:.2f}->{new_L:.2f} cm, W {table_W:.2f}->{new_W:.2f} cm (margin={margin_cm}cm)")
        return [new_L, new_W, table_H], True
    else:
        print("[Auto-Fit] All objects are within the table, no extension needed.")
        return adjusted_main_size, False
...
def calculate_pose_main(output_assets_dir):
    ...
    try:
        ...
        # 6.5 Table enlargement adaptation (if needed), and adjust pose as required
        print("\n6.5. Auto-fitting table to contain all rotated objects...")
        old_main_size = list(adjusted_main_size)  # Record size before extension
        adjusted_main_size, changed = fit_table_to_objects(
            adjusted_main_size=adjusted_main_size,
            main_object_name=main_object_name,
            size_data=size_data,
            rotation_data=rotation_data,
            poses=poses,
            margin_cm=FIT_MARGIN_CM
        )
        if changed and ADAPT_POSES_WITH_TABLE_RESIZE:
            print("Table extended, scaling object positions in local coordinate system according to table ratio...")
            poses = rescale_poses_for_table_resize(
                old_main_size, adjusted_main_size, main_object_name, rotation_data, poses
            )
        elif changed:
            print("Table extended, but keeping absolute object positions unchanged (behavior when position adaptation is off).")

        # 7. Build final result (using potentially enlarged table size)
        final_vlm_pose = {
